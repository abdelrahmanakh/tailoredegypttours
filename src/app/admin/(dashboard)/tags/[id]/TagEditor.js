...
export default function TagEditor({ tag, languages, saveAction, updateSharedAction }) {
  const [activeLangId, setActiveLangId] = useState(languages[0]?.id)
  
  const activeLang = languages.find(l => l.id === activeLangId)
  const translation = tag.translations.find(t => t.languageId === activeLangId) || {}

  const [drafts, setDrafts] = useState(() => {
    const initial = {}
    languages.forEach(lang => {
      const found = tag.translations.find(t => t.languageId === lang.id)
      initial[lang.id] = {
        name: found?.name || '',
  ...
  const [sharedDraft, setSharedDraft] = useState({
    ...
    featuredOrder: tag.featuredOrder ?? 0
  })
  ...
  const isSharedDirty = 
    sharedDraft.slug !== tag.slug ||
    sharedDraft.isFeatured !== tag.isFeatured ||
    // Use loose equality (==) for numbers because inputs return strings ('5' vs 5)
    sharedDraft.featuredOrder != (tag.featuredOrder ?? 0);
    ...
    <div className="space-y-8">
      ...
      <form action={handleSharedSave} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div className="flex justify-between items-center border-b pb-4 mb-6">
          ...
          <span className={`text-xs font-bold px-2 py-1 rounded ${tag.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                {tag.isActive ? 'Active' : 'Draft'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div>
            ...
            <input name="slug" defaultValue={tag.slug} className="w-full border border-gray-300 rounded-lg p-2.5 font-mono text-sm bg-gray-50" />
          </div>
        </div>
